...
class Deck {
    private $cards=[];
    public function generateFromFolder(string $folder, int $pairs, string $publicPrefix=null) {
        $publicPrefix = $publicPrefix ?? $folder;
        ...
        foreach ($chosen as $i=>$file) {
            $id=$i+1;
            $publicPath = rtrim($publicPrefix, '/\\') . '/' . $file;
            $tmp[] = new Card($id, $publicPath, $idx++);
            $tmp[] = new Card($id, $publicPath, $idx++);
        }
        shuffle($tmp);
